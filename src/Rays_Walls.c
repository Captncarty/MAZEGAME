...
void drawRays(){
    glColor3f(0,1,1);
    glBegin(GL_QUADS);  glVertex2i(526, 0); glVertex2i(1006, 0);    glVertex2i(1006, 160);  glVertex2i(526, 160); 
    glEnd(); 

    glColor3f(0,0,1);
    glBegin(GL_QUADS);  glVertex2i(526, 160);   glVertex2i(1006, 160);  glVertex2i(1006, 320);  glVertex2i(526, 320); 
    glEnd();
    ...
    for (r = 0; r < 60; r++){
        ...
        while (dof < 8){
            ...
            if (mp > 0 && mp < mapX*mapY && mapW[mp] > 0){
                vmt = mapW[mp] - 1; dof = 8;    disV = cos(getRad(ra)) * (rx - px) - sin(getRad(ra)) * (ry - py);   //hit
            }else {
                rx += xo;   ry += yo;   dof += 1;   //next horizontal
            }
        }
        ...
        while (dof < 8){
            mx = (int)(rx) >> 6;    my = (int)(ry) >> 6;    mp = my*mapX + mx;
            if (mp > 0 && mp < mapX*mapY && mapW[mp] > 0){
                hmt = mapW[mp] - 1; dof = 8;    disH = cos(getRad(ra)) * (rx - px) - sin(getRad(ra)) * (ry - py);   //hit
            }else {
                rx += xo;   ry += yo;   dof += 1;
            }   //next horizontal
        }
        ...
        for (y = 0; y < lineH; y++){
            ...
            glPointSize(8); glBegin(GL_POINTS); glVertex2i(r * 8 + 530, y + lineOff);
        ...
        for (y = lineOff + lineH; y < 320; y++){
            ...
            glColor3f(c/1.3, c/1.3, c);    glPointSize(8); glBegin(GL_POINTS); glVertex2i(r*8 + 530, y);
            glEnd();
